/**
 * Public Booking JavaScript - Direct Table Booking
 * Handles the public table booking form functionality when a specific table is pre-selected.
 * This file is loaded only for /booktable/{slug}.
 */
...
function initializePublicBooking(config) {
    ...
    config.initialTableId = config.initialTableId || 0;
    config.initialFloorId = config.initialFloorId || 0;
    config.lockTableSelection = true; // Always lock for direct table booking
...
// Entry point used by view
function initializePublicBookingForTable(config) {
    initializePublicBooking(config || {});
}

// --- The rest of the logic is identical to PublicBooking.js but assumes lockTableSelection=true ---

// (Copy of core functions from PublicBooking.js)
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.Status == 1 && response.Data && response.Data.TimeSlots) {
                // For direct table booking, verify the specific table's availability for each slot
                if (publicBookingConfig && publicBookingConfig.initialTableId && publicBookingConfig.initialTableId > 0) {
                    verifySpecificTableAvailability(response.Data.TimeSlots, response.Data.BookingDate);
                } else {
...
function verifySpecificTableAvailability(timeSlots, bookingDate) {
    // For direct table booking, check if the specific table is available for each time slot
    var tableId = publicBookingConfig.initialTableId;
    var noOfGuests = parseInt($('#noOfGuests').val()) || 0;
    var duration = publicBookingConfig.defaultDuration || 120;
    ...
    var checkPromises = timeSlots.map(function(slot) {
        var timeValue = typeof slot.TimeValue === 'string' ? slot.TimeValue : String(slot.TimeValue || '');
        
        return $.ajax({
            url: '/api/PublicTableBooking/CheckTableAvailability',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                Slug: publicBookingConfig.slug,
                BookingDate: bookingDate,
                BookingTimeString: timeValue,
                TableIds: [tableId],
                NoOfGuests: noOfGuests,
                Duration: duration
            })
        }).then(function(checkResponse) {
            var isTableAvailable = checkResponse.Status == 1 && 
                                  checkResponse.Data && 
                                  checkResponse.Data.IsAvailable === true;
            
            // A slot is only available if BOTH: general availability AND specific table availability
            return {
                Time: slot.Time,
                TimeValue: slot.TimeValue,
                IsAvailable: slot.IsAvailable && isTableAvailable
            };
        }).catch(function() {
    ...
    // Wait for all checks to complete, then display
    $.when.apply($, checkPromises).then(function() {
        var verifiedSlots = Array.prototype.slice.call(arguments);
        displayTimeSlots(verifiedSlots);
    });
}
...
function displayTimeSlots(timeSlots) {
    ...
    // Floor selector is not needed in direct table mode
    $('#floorGroup').hide();
    hideLoading();
}
...
function renderPublicTableLayout() {
    ...
    // We don't expose floor dropdown in direct table mode, so we skip binding it here
    
    // Just render all tables (layout still shows other tables as context)
    var tablesToShow = [];
    $.each(publicTableDataCache, function(tableId, table) {
...
function selectPublicTable(tableId) {
    ...
    // In direct table mode, always keep only the primary table selected
    clearTableSelection();
    ...
    // Update hidden field with comma-separated IDs (for backward compatibility)
...
function updateSelectedTablesDisplay() {
    // For direct table booking, we don't need the bottom
    // "Selected / Total Capacity / Clear" summary bar, so hide it.
...
function displayTables(tables) {
    // If this page is for a specific table, keep only that table in the UI
    if (publicBookingConfig && publicBookingConfig.initialTableId && publicBookingConfig.initialTableId > 0) {
        tables = tables.filter(function (t) { return t.TableId === publicBookingConfig.initialTableId; });
    }

    // Store tables in cache for layout view (after filtering)
    ...
    if (tables.length === 0) {
        ...
        // Keep entire table selection panel hidden for direct table booking
        $('#tableGroup').hide();
        $('#tableLayoutCard').hide();
        $('#tablesContainer').hide();
        hideLoading();
    ...
    // Default: Show layout view, hide list view but keep container hidden
    $('#tableLayoutCard').show();
    $('#tablesContainer').hide();
    $('#toggleViewBtn').html('<i class="fas fa-list"></i>').attr('title', 'Show List View');
    
    // Hide the entire "Select Table(s)" group for direct table booking
    $('#tableGroup').hide();
    hideLoading();
