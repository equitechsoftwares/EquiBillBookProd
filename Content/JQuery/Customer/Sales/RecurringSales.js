$(function () {
    ...
    $('#_StartDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
    $('#_EndDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
...
var _RecurringSalesId = 0, Status = '', _CustomerId = 0;
var _BranchId = 0;
...
function fetchList(PageIndex) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/RecurringSales/RecurringSalesFetch',
        datatype: "json",
        data: det,
        type: "post",
...
function deleteCombo(id, RecurringSalesDetailsId) {
    if (RecurringSalesDetailsId != undefined) {
...
function deleteFullCombo(RecurringSalesId) {
    if (RecurringSalesId != undefined) {
        $('#divCombo tr').each(function () {
            var _id = this.id.split('divCombo')[1];
            if (_id != undefined) {
                $('#divCombo' + _id).hide();
...
function insert(i) {
    ...
    var det = {
        ...
        IsActive: true,
        IsDeleted: false,
        RecurringSalesDetails: ItemDetails,
        RecurringSalesType: $('#ddlRecurringSalesType').val(),
        SellingPriceGroupId: $('#ddlSellingPriceGroup').val(),
        ...
        RepeatEveryNumber: $('#txtRepeatEveryNumber').val(),
        RepeatEvery: $('#ddlRepeatEvery').val(),
        StartDate: $('#txtStartDate').val(),
        EndDate: $('#txtEndDate').val(),
        IsNeverExpires: $('#chkIsNeverExpires').is(':checked')
    };
    $("#divLoading").show();
    $.ajax({
        url: '/RecurringSales/RecurringSalesInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (i == 1) {
                    window.location.href = "/Recurringsales/index";
                }
                else {
                    window.location.href = "/Recurringsales/add";
                }
            }
        },
        error: function (xhr) {
...
function update(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            ...
            ItemDetails.push({
                RecurringSalesDetailsId: $('#txtRecurringSalesDetailsId' + _id).val(),
                DivId: _id,
                Quantity: $('#txtQuantity' + _id).val(),
                AmountExcTax: $('#txtAmountExcTax' + _id).val(),
                AmountIncTax: $('#txtAmountIncTax' + _id).val(),
    ...
    $('#divAdditionalCharges .additional-charges-row').each(function (index) {
        ...
        var additionalChargeId = $('#txtAdditionalChargeId' + count).val();
        var taxExemptionId = $('#exemptionReasonId' + count).val();
        var recurringSalesAdditionalChargesId = $('#txtRecurringSalesAdditionalChargesId' + count).val();
        additionalCharges.push({
            AdditionalChargeId: additionalChargeId,
            TaxId: taxId,
            AmountExcTax: additionalChargesAmountExcTax,
            AmountIncTax: additionalChargesAmountIncTax,
            TaxExemptionId: taxExemptionId,
            RecurringSalesAdditionalChargesId: recurringSalesAdditionalChargesId,
            IsActive: true,
            IsDeleted: false
        })
    });
    ...
    var det = {
        RecurringSalesId: window.location.href.split('=')[1],
        IsReverseCharge: $('#chkIsReverseCharge').is(':checked') == true ? 1 : 2,
        GrandTotalReverseCharge: $("#divGrandTotal_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
        RoundOffReverseCharge: $("#divRoundOff_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
        NetAmountReverseCharge: $("#divNetAmount_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/RecurringSales/RecurringSalesUpdate',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function Delete(RecurringSalesId) {
    var r = confirm("This will delete permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            RecurringSalesId: RecurringSalesId,
        };
        $("#divLoading").show();
        $.ajax({
            url: '/RecurringSales/RecurringSalesdelete',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function Stop(RecurringSalesId) {
    var r = confirm("This will stop permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            RecurringSalesId: RecurringSalesId,
        };
        $("#divLoading").show();
        $.ajax({
            url: '/RecurringSales/RecurringSalesStop',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function View(RecurringSalesId) {
    var det = {
        RecurringSalesId: RecurringSalesId
    };
    $("#divLoading").show();
    $.ajax({
        url: '/RecurringSales/RecurringSalesView',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (SubmitType == 'insert') {
                    ...
                    UpdateSalesStatus(_RecurringSalesId);
                }
            }
...
function toggleNeverExpires() {
    if ($('#chkIsNeverExpires').is(':checked') == true) {
        $('.divEndDate').hide();
    }
    else {
        $('.divEndDate').show();
