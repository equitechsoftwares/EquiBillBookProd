$(function () {
    ...
    $('#_DeliveryChallanDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
    $('#_PaymentDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat });
    $('#_DeliveryChallanDate').addClass('notranslate');
...
var _DeliveryChallanId = 0, Status = '';
var _BranchId = 0;
...
function fetchList(PageIndex) {
    ...
    var det = {
        ...
        DeliveryChallanType: $('#ddlDeliveryChallanType').val(),
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/DeliveryChallan/DeliveryChallanFetch',
        datatype: "json",
        data: det,
        type: "post",
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.Data.ItemDetails.length; i++) {
                ...
                if (isPresent == false) {
                    ...
                    var ddlTax = '<select class="form-control select2 ddlTax" style="min-width:80px" id="ddlTax' + count + '" onchange="ChangeQtyAmount(' + count + ')">';
...
function deleteFullCombo(DeliveryChallanId) {
    if (DeliveryChallanId != undefined) {
        //var r = confirm("This will delete permanently. This process cannot be undone. Are you sure you want to continue?");
        //if (r == true) {
        $('#divCombo tr').each(function () {
            var _id = this.id.split('divCombo')[1];
...
function insert(i) {
    ...
    var det = {
        ...
        DeliveryChallanDate: moment($("#txtDeliveryChallanDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),//$("#txtDeliveryChallanDate").val(),
        Status: $("#ddlStatus").val(),
        ...
        IsActive: true,
        IsDeleted: false,
        DeliveryChallanDetails: ItemDetails,
        DeliveryChallanType: $("#ddlDeliveryChallanType").val(),
        SellingPriceGroupId: $('#ddlSellingPriceGroup').val(),
        ...
        DeliveryChallanAdditionalCharges: additionalCharges,
        SpecialDiscount: $('#txtSpecialDiscount').val()
    };
    $("#divLoading").show();
    $.ajax({
        url: '/DeliveryChallan/DeliveryChallanInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                /*if (Status == 'Pending') {*/
                if (data.Data.SaleSetting.AutoPrintInvoiceDeliveryChallan == true) {
                        sessionStorage.setItem('InvoiceUrl', '/DeliveryChallan/invoice?InvoiceId=' + data.Data.DeliveryChallan.InvoiceId);
                    }
                //}
                //else if (Status == 'Sent') {
                ...
                if (i == 1) {
                    window.location.href = "/DeliveryChallan/index";
                }
                else {
                    window.location.href = "/DeliveryChallan/add";
                }
...
function update(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            ...
            ItemDetails.push({
                DeliveryChallanDetailsId: $('#txtDeliveryChallanDetailsId' + _id).val(),
                DivId: _id,
                Quantity: $('#txtQuantity' + _id).val(),
                AmountExcTax: $('#txtAmountExcTax' + _id).val(),
                AmountIncTax: $('#txtAmountIncTax' + _id).val(),
    ...
    $('#divAdditionalCharges .additional-charges-row').each(function (index) {
        ...
        var additionalChargeId = $('#txtAdditionalChargeId' + count).val();
        var taxExemptionId = $('#exemptionReasonId' + count).val();
        var deliveryChallanAdditionalChargesId = $('#txtDeliveryChallanAdditionalChargesId' + count).val();
        additionalCharges.push({
            AdditionalChargeId: additionalChargeId,
            TaxId: taxId,
            AmountExcTax: additionalChargesAmountExcTax,
            AmountIncTax: additionalChargesAmountIncTax,
            TaxExemptionId: taxExemptionId,
            DeliveryChallanAdditionalChargesId: deliveryChallanAdditionalChargesId,
            IsActive: true,
            IsDeleted: false
        })
    });
    ...
    var det = {
        DeliveryChallanId: window.location.href.split('=')[1],
        IsReverseCharge: $('#chkIsReverseCharge').is(':checked') == true ? 1 : 2,
        GrandTotalReverseCharge: $("#divGrandTotal_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
        RoundOffReverseCharge: $("#divRoundOff_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
        NetAmountReverseCharge: $("#divNetAmount_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/DeliveryChallan/DeliveryChallanUpdate',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function Delete(DeliveryChallanId) {
    var r = confirm("This will delete permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            DeliveryChallanId: DeliveryChallanId,
        ...
        $("#divLoading").show();
        $.ajax({
            url: '/DeliveryChallan/DeliveryChallandelete',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function Cancel(DeliveryChallanId) {
    ...
    if (r == true) {
        ...
        $("#divLoading").show();
        $.ajax({
            url: '/DeliveryChallan/DeliveryChallanCancel',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function View(DeliveryChallanId) {
    var det = {
        DeliveryChallanId: DeliveryChallanId
    };
    $("#divLoading").show();
    $.ajax({
        url: '/DeliveryChallan/DeliveryChallanView',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function UpdateSalesStatus() {
    ...
    var det = {
        DeliveryChallanId: _DeliveryChallanId,
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (data.Data.SaleSetting.AutoPrintInvoiceDeliveryChallan == true) {
                    PrintInvoice('/DeliveryChallan/invoice?InvoiceId=' + data.Data.DeliveryChallan.InvoiceId);
                }
            }
        },
        error: function (xhr) {
...
function AcceptDeclineSalesStatus(_id, Status) {
    var r = confirm("This will mark the Delivery Challan as " + Status + ". This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        $('.errorText').hide();
        $('[style*="border: 2px"]').css('border', '');
