$(function () {
    ...
    $('#_SalesQuotationDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
    $('#_PaymentDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat });
    $('#_SalesQuotationDate').addClass('notranslate');
...
var _SalesQuotationId = 0, Status = '';
var _BranchId = 0;
...
function fetchList(PageIndex) {
    ...
    var det = {
        ...
        SalesQuotationType: $('#ddlSalesQuotationType').val(),
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/SalesQuotation/SalesQuotationFetch',
        datatype: "json",
        data: det,
        type: "post",
            ...
            for (let i = 0; i < data.Data.ItemDetails.length; i++) {
                ...
                if (isPresent == false) {
                    ...
                    if (data.Data.ItemDetails[i].VariationName) {
                        variation = '</br><b> <b>Variation :</b> ' + data.Data.ItemDetails[i].VariationName;
                    }
...
function deleteFullCombo(SalesQuotationId) {
...
function insert(i) {
    ...
    var det = {
        ...
        SalesQuotationDate: moment($("#txtSalesQuotationDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),//$("#txtSalesQuotationDate").val(),
        Status: $("#ddlStatus").val(),
        ...
        IsActive: true,
        IsDeleted: false,
        SalesQuotationDetails: ItemDetails,
        SalesQuotationType: $("#ddlSalesQuotationType").val(),
        SellingPriceGroupId: $('#ddlSellingPriceGroup').val(),
        ...
        SalesQuotationAdditionalCharges: additionalCharges,
        SpecialDiscount: $('#txtSpecialDiscount').val()
    };
    $("#divLoading").show();
    $.ajax({
        url: '/SalesQuotation/SalesQuotationInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (data.Data.SaleSetting.AutoPrintInvoiceQuotation == true) {
                    sessionStorage.setItem('InvoiceUrl', '/SalesQuotation/invoice?InvoiceId=' + data.Data.SalesQuotation.InvoiceId);
                }

                if (i == 1) {
                    window.location.href = "/SalesQuotation/index";
                }
                else {
                    window.location.href = "/SalesQuotation/add";
                }
            }
        },
        error: function (xhr) {
...
function update(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            ...
            ItemDetails.push({
                SalesQuotationDetailsId: $('#txtSalesQuotationDetailsId' + _id).val(),
                DivId: _id,
                Quantity: $('#txtQuantity' + _id).val(),
                AmountExcTax: $('#txtAmountExcTax' + _id).val(),
                AmountIncTax: $('#txtAmountIncTax' + _id).val(),
    ...
    $('#divAdditionalCharges .additional-charges-row').each(function (index) {
        ...
        var additionalChargeId = $('#txtAdditionalChargeId' + count).val();
        var taxExemptionId = $('#exemptionReasonId' + count).val();
        var salesQuotationAdditionalChargesId = $('#txtSalesQuotationAdditionalChargesId' + count).val();
        additionalCharges.push({
            AdditionalChargeId: additionalChargeId,
            TaxId: taxId,
            AmountExcTax: additionalChargesAmountExcTax,
            AmountIncTax: additionalChargesAmountIncTax,
            TaxExemptionId: taxExemptionId,
            SalesQuotationAdditionalChargesId: salesQuotationAdditionalChargesId,
            IsActive: true,
            IsDeleted: false
        })
    });
    ...
    var det = {
        SalesQuotationId: window.location.href.split('=')[1],
        IsReverseCharge: $('#chkIsReverseCharge').is(':checked') == true ? 1 : 2,
        GrandTotalReverseCharge: $("#divGrandTotal_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
        RoundOffReverseCharge: $("#divRoundOff_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
        NetAmountReverseCharge: $("#divNetAmount_reversecharge").text().replace(/[^-0-9\.]+/g, ""),
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/SalesQuotation/SalesQuotationUpdate',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function Delete(SalesQuotationId) {
    var r = confirm("This will delete permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            SalesQuotationId: SalesQuotationId,
        ...
        $("#divLoading").show();
        $.ajax({
            url: '/SalesQuotation/SalesQuotationdelete',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function View(SalesQuotationId) {
    var det = {
        SalesQuotationId: SalesQuotationId
    };
    $("#divLoading").show();
    $.ajax({
        url: '/SalesQuotation/SalesQuotationView',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function UpdateSalesStatus() {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/SalesQuotation/UpdateSalesQuotationStatus',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (data.Data.SaleSetting.AutoPrintInvoiceQuotation == true) {
                    PrintInvoice('/SalesQuotation/invoice?InvoiceId=' + data.Data.SalesQuotation.InvoiceId);
                }
            }
        },
        error: function (xhr) {
...
function AcceptDeclineSalesStatus(_id, Status) {
    var r = confirm("This will mark the Sales Quotation as " + Status + ". This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        $('.errorText').hide();
        $('[style*="border: 2px"]').css('border', '');
