...
var _SalesId = 0, Status = '', _CustomerId = 0;
var _BranchId = 0;
...
function fetchTaxExemptions(callback) {
    var det = {
...
function insert(i) {
    ...
    var det = {
        ...
        SalesType: IsBillOfSupply == false ? 'Sales' : "Bill Of Supply",
        SellingPriceGroupId: $('#ddlSellingPriceGroup').val(),
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (i == 1) {
                    ...
                    if (IsBillOfSupply == false) {
                        window.location.href = "/sales/add";
                    }
                    else {
                        window.location.href = "/sales/billofsupplyadd";
                    }
                }
            }
        },
...
function insertInnerTax() {
    ...
    var det = {
        ...
        SupplierPaymentAccountId: $('#ddlSupplierPayment_N').val(),
...
function openWriteOffModal(SalesId) {
    ...
    $('#_WriteOffDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });

    $('#_WriteOffDate').addClass('notranslate');
}
...
function UpdateWriteOff() {
    ...
    var det = {
        SalesId: _SalesId,
        WriteOffDate: moment($("#txtWriteOffDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),
        WriteOffReason: $('#txtWriteOffReason').val()
    };
    $("#divLoading").show();
    $.ajax({
...
function openAvailableCredits(SalesId, CustomerId, GrandTotal, Due, InvoiceNo) {
    $("#paymentModal").modal('hide');
    _SalesId = SalesId;
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined && $('#txtAmount' + _id).val() != '') {
            CustomerPaymentIds.push({
                Type: 'Sales Payment',
                CustomerPaymentId: $('#hdnCustomerPaymentId' + _id).val(),
...
function deleteShippingBill(ShippingBillId) {
    var r = confirm("Are you sure you want to delete?");
    if (r == true) {
        var det = {
...
function insertItem(i) {
    ...
    if ($('#ddlProductType').val() == 'Single') {
        ...
        var variationDetails = [];
        $('#divComboItems tr').each(function () {
            var _id = this.id.split('divCombo')[1];
            if (_id != undefined) {
...
function updatePointsEarnedInfoOnTotalChange() {
    console.log('[Sales] updatePointsEarnedInfoOnTotalChange called');
    if (typeof updatePointsEarnedInfo === 'function') {
        ...
        updatePointsEarnedInfoTimeout = setTimeout(function() {
            console.log('[Sales] Debounced updatePointsEarnedInfo triggered');
            updatePointsEarnedInfo();
        }, 500); // Increased from immediate call to 500ms to reduce excessive calls
    } else {
        console.warn('[Sales] updatePointsEarnedInfo function not found');
    }
}

var isUpdatingPointsEarned = false;
function updatePointsEarnedInfo() {
    console.log('[Sales] updatePointsEarnedInfo called');
    
    if (!rewardPointsSetting || !rewardPointsSetting.EnableRewardPoint) {
        console.log('[Sales] Reward points not enabled or setting not available');
    ...
    if (pointsEarnedAjaxRequest && pointsEarnedAjaxRequest.readyState !== 4) {
        console.log('[Sales] Cancelling previous CalculatePointsEarned AJAX request');
    ...
    if (isUpdatingPointsEarned) {
        console.log('[Sales] Already updating points earned, skipping...');
        return;
    }
    ...
    console.log('[Sales] Order calculation:', {
        grandTotal: grandTotal,
        pointsDiscount: pointsDiscount,
        orderAmountForPoints: orderAmountForPoints
    });
    ...
    if (rewardPointsSetting.MinOrderTotalToEarnReward > 0 && orderAmountForPoints < rewardPointsSetting.MinOrderTotalToEarnReward) {
        console.log('[Sales] Order amount below minimum:', orderAmountForPoints, '<', rewardPointsSetting.MinOrderTotalToEarnReward);
        $('#pointsEarnedInfo').hide();
    ...
    console.log('[Sales] Making AJAX call to CalculatePointsEarned:', {
        companyId: companyId,
        orderAmount: orderAmountForPoints,
        cookieData: cookieData ? cookieData.substring(0, 100) + '...' : 'null'
    });
    
    if (!companyId) {
        console.error('[Sales] CompanyId is null or invalid, falling back to local calculation');
    ...
    pointsEarnedAjaxRequest = $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            console.log('[Sales] CalculatePointsEarned success response:', data);
            ...
            if (data.Status == 1 && data.Data) {
                ...
                console.log('[Sales] Points earned from server:', pointsEarned);
                if (pointsEarned > 0) {
                    $('#pointsEarned').text(pointsEarned.toFixed(0));
                    $('#pointsEarnedInfo').show();
                    console.log('[Sales] Updated points earned display to:', pointsEarned);
                } else {
                    $('#pointsEarnedInfo').hide();
                    console.log('[Sales] No points earned, hiding display');
                }
            } else {
                console.log('[Sales] Invalid response status or data:', data);
                $('#pointsEarnedInfo').hide();
            }
            isUpdatingPointsEarned = false;
            pointsEarnedAjaxRequest = null;
        ...
        error: function (xhr, status, error) {
            ...
            if (status !== 'abort') {
                console.error('[Sales] CalculatePointsEarned AJAX error:', {
                    status: status,
                    error: error,
                ...
                console.log('[Sales] CalculatePointsEarned AJAX request was aborted');
                isUpdatingPointsEarned = false;
                pointsEarnedAjaxRequest = null;
                return;
            }
            ...
            console.log('[Sales] Using fallback calculation, points earned:', pointsEarned);
            if (pointsEarned > 0) {
                $('#pointsEarned').text(pointsEarned.toFixed(0));
                $('#pointsEarnedInfo').show();
            } else {
