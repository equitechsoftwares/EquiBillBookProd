function searchSales() {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/tablebooking/searchsalesforlinking',
        datatype: "json",
        data: det,
        type: "post",
...
function linkSales(salesId) {
    if (!confirm('Are you sure you want to link this sales invoice to the booking?')) {
        return;
    }

    // bookingId is defined as a JavaScript variable in the view
    if (typeof bookingId === 'undefined' || !bookingId || bookingId == 0) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/tablebooking/linktosalesaction',
        datatype: "json",
        data: {
            bookingId: bookingId,
            salesId: salesId
