$(function () {
    ...
    $('#_PurchaseDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
...
var _PurchaseId = 0, _CheckStockPriceMismatch = true, _i, taxList;
var _BranchId = 0;
var accountSubTypes;
...
function ViewPayment(SupplierPaymentId) {
    var det = {
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.Data.ItemDetails.length; i++) {
                ...
                if (isPresent == false) {
                    ...
                    var ddlUnit = '<select class="form-control select2" id="ddlUnit' + count + '" onchange="toggleUnit(' + count + ')">';
                    ...
                    var ddlTax = '<select class="form-control select2 ddlTax" id="ddlTax' + count + '" onchange="ChangeQtyAmount(' + count + ')" style="min-width:80px">';
                    ...
                    html = html + '<tr id="divCombo' + count + '">' +
                        ...
                        '</td>' +
                        '<td style="min-width:100px">' +
                        '<input type="number"' + (EnableEditingProductPrice == 'true' ? '' : 'disabled') + ' class="form-control" id="txtSalesExcTax' + count + '"  value="' + data.Data.ItemDetails[i].SalesExcTax + '">' +
                        //'<input type="number"' + (EnableEditingProductPrice == 'true' ? '' : 'disabled') + ' class="form-control" id="txtSalesIncTax' + count + '"  value="' + data.Data.ItemDetails[i].SalesIncTax + '" onchange="updateSalePrice(' + count + ')">' +
                        '</td>' +
...
function deleteFullCombo(PurchaseId) {
...
function discallcalc() {
    ...
    let divOtherCharges = $('#txtOtherCharges').val() == '' ? 0 : parseFloat($('#txtOtherCharges').val());
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                let taxamt = (amountIncTax - amountExcTax) + (parseFloat(divTotalAmount - (amount + discount + _extraDiscount)) * (taxper / 100));
                //$('#hdndivTax').val((parseFloat(divTotalAmount - (amount + discount)) * (taxper / 100)));
                //$('#hdndivTotalTax').val(taxamt);
                ...
                var grandTotal = Math.round(((divTotalAmount + taxamt + divOtherCharges + PackagingCharge + ShippingCharge) - parseFloat($('#hdndivDiscount').val())) * 100) / 100;
...
function insert(i) {
    ...
    var det = {
        ParentId: $('#ddlPurchaseInvoice').val(),
        BranchId: $('#ddlBranch').val(),
        ...
        PurchaseDate: moment($("#txtPurchaseDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),
        Status: $("#ddlPurchaseStatus  option:selected").val(),
        ReferenceNo: $("#txtPurchaseReferenceNo").val(),
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                /*if ($("#ddlPurchaseStatus option:selected").val() == 'Received') {*/
                if (data.Data.PurchaseSetting.AutoPrintInvoicePurchaseBill == true) {
                    sessionStorage.setItem('InvoiceUrl', '/purchase/invoice?InvoiceId=' + data.Data.Purchase.InvoiceId);
                }
                //}
...
function update(i) {
    ...
    var det = {
        PurchaseId: window.location.href.split('=')[1].replace('#', ''),
        ParentId: $('#ddlPurchaseInvoice').val(),
        BranchId: $('#ddlBranch').val(),
        DeliveredTo: $('#txtDeliveredTo').val(),
        Discount: $("#txtDiscAll").val(),
...
function insertPayment() {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/purchase/PuchasePaymentInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function UpdatePurchaseStatus(id) {
    var r = confirm("This will mark the Purchase Bill as Received. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        $('.errorText').hide();
        $('[style*="border: 2px"]').css('border', '');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.Status == 0) {
                    ...
                    if (data.Data.PurchaseSetting.AutoPrintInvoicePurchaseBill == true) {
                        PrintInvoice('/purchase/invoice?InvoiceId=' + data.Data.Purchase.InvoiceId);
                    }
                }
            },
            error: function (xhr) {
...
function AddNewVariation() {
    ...
    var html = '<table class="table table-bordered add-product-price-table table-condensed" id="product_variation_form_part' + count + '">' +
        ...
        '<thead>' +
        '<tr>' +
        '<th class="divInventoryAccount ' + ((IsPurchaseAddon == 'true' && IsAccountsAddon == 'true') ? '' : 'hidden') + '">Inventory Account </th>' +
        '<th>SKU * <i tabindex="0" class="fa fa-info-circle text-info hover-q no-print ' + (ShowHelpText == "True" ? "" : "hidden") + '" role="button" data-toggle="popover" data-trigger="focus" title="" data-content="Leave blank to auto generate"></i></th>' +
        '<th>Value *</th>' +
        '<th class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '">' +
        'Default Purchase Price *<br> <small> <i><span class="dsp_label ">Exc. Tax/Inc. Tax</span></i></small>' +
        '</th>' +
        '<th class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '"> x Margin(%) *</th>' +
        '<th class="">' +
        'Default Selling Price *<br>' +
        '<small> <i><span class="dsp_label lblTaxName" >Exc. Tax</span></i></small>' +
        '</th>' +
        '<th class="' + (EnableMrp == 'true' ? '' : 'hidden') + '"> Default MRP *</th>' +
        ...
        '<td style="min-width:200px" class="divInventoryAccount ' + ((IsPurchaseAddon == 'true' && IsAccountsAddon == 'true') ? '' : 'hidden') + '">' +
        '<div class="row">' +
        '<div class="col-sm-12" >' +
        ...
        '</td>' +
        '<td style="min-width:500px" class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '">' +
        '<div class="row">' +
        '<div class="col-6 ">' +
        '<input class="form-control input-sm variable_dpp input_number" placeholder="Exc. Tax" required="" name="product_variation" type="number" id="txtPurchaseExcTax' + innerCount + '" onchange="UpdateVariationAmount(1,' + innerCount + ')">' +
        ...
        '</div>' +
        '<div class="col-sm-6 mt-2 ' + (IsAccountsAddon == 'true' ? '' : 'hidden') + '" >' +
        '<div class="form-group">' +
        '<label> Purchase Account<span class="danger">*</span></label>' +
        '<div class="input-group">' +
        '<select style="min-width:100px" class="form-control select2" id="ddlPurchaseAccount' + innerCount + '">' +
        ...
        '</div>' +
        '</td >' +
        '<td style="min-width:200px" class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '">' +
        '<input class="form-control input-sm variable_profit_percent input_number" required="" name="product_variation" type="number" value="' + ($('#hdnDefaultProfitMargin').val() > 0 ? $('#hdnDefaultProfitMargin').val() : "") + '" placeholder="" id="txtDefaultProfitMargin' + innerCount + '" onchange="UpdateVariationAmount(3,' + innerCount + ')">' +
        ...
        '<input class="form-control input-sm variable_dsp input_number divSellingPriceExcTax" placeholder="" required="" name="product_variation" type="number" id="txtSalesExcTax' + innerCount + '" onchange="UpdateVariationAmount(4,' + innerCount + ')">' +
        '<small class="text-red font-weight-bold errorText" id="divSalesTax' + innerCount + '"></small>' +
        '</div>' +
        '<div class="col-12 divSellingPriceIncTax">' +
        '<input class="form-control input-sm variable_dsp input_number divSellingPriceIncTax" placeholder="" required="" name="product_variation" type="number" id="txtSalesIncTax' + innerCount + '" onchange="UpdateVariationAmount(5,' + innerCount + ')">' +
        '<small class="text-red font-weight-bold errorText" id="divSalesTax' + innerCount + '"></small>' +
        '</div>' +
        '<div class="col-sm-12 mt-2 ' + (IsAccountsAddon == 'true' ? '' : 'hidden') + '">' +
...
function AddNewInnerVariation(id) {
    ...
    var html = '<tr id="product_variation_form_part_inner' + innerCount + '">' +
        ...
        '</td>' +
        '<td style="min-width:200px">' +
        '<input class="form-control input-sm input_sub_sku" name="product_variation[0][variations][0][sub_sku]" type="text" id="txtSku' + innerCount + '">' +
        '<small class="text-red font-weight-bold errorText" id="divSku' + innerCount + '"></small>' +
        '</td>' +
        '<td style="min-width:200px">' +
        '<input class="form-control input-sm variation_value_name" required="" name="product_variation" type="text" value="" id="txtValue' + innerCount + '"> ' +
        '<input name="product_variation[0][variations][0][variation_value_id]" type="hidden" id="hdnValue' + innerCount + '" value="0">' +
        '</td>' +
        '<td style="min-width:500px" class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '">' +
        '<div class="row">' +
        '<div class="col-6 ">' +
        '<input class="form-control input-sm variable_dpp input_number" required="" name="product_variation" type="number" id="txtPurchaseExcTax' + innerCount + '" onchange="UpdateVariationAmount(1,' + innerCount + ')">' +
        '<small class="text-red font-weight-bold errorText" id="divPurchaseExcTax' + innerCount + '"></small>' +
        '</div>' +
        '<div class="col-6 ">' +
        ...
        '</div>' +
        '</td >' +
        '<td class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '" style="min-width:200px" >' +
        '<div class="input-group">' +
        '<input class="form-control input-sm variable_profit_percent input_number" required="" name="product_variation" type="number" value="" id="txtDefaultProfitMargin' + innerCount + '" onchange="UpdateVariationAmount(3,' + innerCount + ')">' +
        '<small class="text-red font-weight-bold errorText" id="divDefaultProfitMargin' + innerCount + '"></small>' +
        '</div>' +
        '</td>' +
        '<td style="min-width:200px">' +
        '<div class="row">' +
        '<div class="col-12 divSellingPriceExcTax">' +
        '<input class="form-control input-sm variable_dsp input_number divSellingPriceExcTax" required="" name="product_variation" type="number" id="txtSalesExcTax' + innerCount + '" onchange="UpdateVariationAmount(4,' + innerCount + ')">' +
        '<small class="text-red font-weight-bold errorText" id="divSalesTax' + innerCount + '"></small>' +
        '</div>' +
        '<div class="col-12 divSellingPriceIncTax">' +
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.Data.VariationDetails.length; i++) {
                html = html + '<tr id="product_variation_form_part_inner' + innerCount + '">' +
                    ...
                    '</td>' +
                    '<td style="min-width:200px">' +
                    '<input class="form-control input-sm variation_value_name" required="" disabled name="product_variation" type="text" id="txtValue' + innerCount + '" value="' + data.Data.VariationDetails[i].VariationDetails + '">' +
                    ...
                    '</td >' +
                    '<td class="' + (IsPurchaseAddon == 'true' ? '' : 'hidden') + '" style="min-width:200px;">' +
                    '<input class="form-control input-sm variable_profit_percent input_number" required="" name="product_variation" type="number" value="' + ($('#hdnDefaultProfitMargin').val() > 0 ? $('#hdnDefaultProfitMargin').val() : "") + '" id="txtDefaultProfitMargin' + innerCount + '" onchange="UpdateVariationAmount(3,' + innerCount + ')">' +
                    '<small class="text-red font-weight-bold errorText" id="divDefaultProfitMargin' + innerCount + '"></small>' +
                    '</td>' +
                    '<td style="min-width:200px">' +
                    '<div class="row">' +
...
function insertItem(i) {
    ...
    if ($('#ddlProductType').val() == 'Single') {
        if (IsPurchaseAddon == 'true') {
            ...
            $('#txtPurchaseExcTax').val($('#txtSalesExcTax').val());
            $('#txtPurchaseIncTax').val($('#txtSalesIncTax').val());
        }
...
$("#_PurchaseDate").on("change.datetimepicker", ({
    date,
    oldDate
}) => {
    if (oldDate != null) {
...
function setDueDate() {
    ...
    var det = {
        PaymentTermId: $("#ddlPaymentTerm").val(),
        InvoiceDate: moment($("#txtPurchaseDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm')
    };
    $("#divLoading").show();
    $.ajax({
...
function openAvailableCredits(PurchaseId, SupplierId, GrandTotal, Due, InvoiceNo) {
    _PurchaseId = PurchaseId;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#AvailableCreditsModal').one('shown.bs.modal', function () {
                var DateFormat = Cookies.get('BusinessSetting').split('&')[0].split('=')[1];
                var TimeFormat = Cookies.get('BusinessSetting').split('&')[1].split('=')[1].replace('tt', 'a');
                if ($('#_PaymentDate').length > 0 && !$('#_PaymentDate').data('DateTimePicker')) {
                    $('#_PaymentDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
                    $('#_PaymentDate').addClass('notranslate');
                }
            });
...
function ApplyCreditsToInvoice() {
    var DateFormat = Cookies.get('BusinessSetting').split('&')[0].split('=')[1];
    var TimeFormat = Cookies.get('BusinessSetting').split('&')[1].split('=')[1].replace('tt', 'a');
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined && $('#txtAmount' + _id).val() != '') {
            SupplierPaymentIds.push({
                Type: 'Purchase Payment',
                SupplierPaymentId: $('#hdnSupplierPaymentId' + _id).val(),
                AmountRemaining: $('#hdnAmountRemaining' + _id).val(),
                Amount: $('#txtAmount' + _id).val(),
                AccountId: $('#hdnAccountId' + _id).val(),
                IsActive: true,
                IsDeleted: false,
            })
        }
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/Purchase/PurchasePaymentInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Status == 0) {
                ...
                var ddlPurchaseInvoice = '<label>Purchase Invoice <span class="danger">*</span> </label> <div class="input-group divPurchaseInvoice_ctrl"><select class="form-control select2" id="ddlPurchaseInvoice">';
