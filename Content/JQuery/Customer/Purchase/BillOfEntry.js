$(function () {
    ...
    $('#_BillOfEntryDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });

    $('#_BillOfEntryDate').addClass('notranslate');
...
function insert(i) {
    ...
    var det = {
        ...
        BillOfEntryDate: moment($("#txtBillOfEntryDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),
        ...
        IsActive: true,
        IsDeleted: false,
        BillOfEntryDetails: ItemDetails,
    };
    $("#divLoading").show();
    $.ajax({
        url: '/Purchase/BillOfEntryInsert',
        datatype: "json",
        data: det,
        type: "post",
...
function update(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            ItemDetails.push({
                BillOfEntryDetailsId: $('#txtBillOfEntryDetailsId' + _id).val(),
                DivId: _id,
                PurchaseDetailsId: $('#txtPurchaseDetailsId' + _id).val(),
                ItemId: $('#txtItemId' + _id).val(),
                ItemDetailsId: $('#txtItemDetailsId' + _id).val(),
    ...
    var det = {
        BillOfEntryId: window.location.href.split('=')[1],
        PurchaseId: $('#hdnPurchaseId').val(),
        SupplierId: $('#hdnSupplierId').val(),
        BillOfEntryNo: $('#txtBillOfEntryNo').val(),
        PortCode: $('#txtPortCode').val(),
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/Purchase/BillOfEntryUpdate',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
