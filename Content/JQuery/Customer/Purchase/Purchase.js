$(function () {
    ...
    // Call the function to check sales context
    checkSalesContext();
...
// Check if URL contains 'sales' parameter to enable/disable dropdowns
function checkSalesContext() {
    var urlParams = new URLSearchParams(window.location.search);
    var typeParam = urlParams.get('type');

    if (!typeParam || (typeParam.toLowerCase().indexOf('purchase') == -1)) {
        // Enable dropdowns for sales context
        $('#ddlSupplier').prop('disabled', false);
        $('#ddlSourceOfSupply').prop('disabled', false);
        $('#ddlDestinationOfSupply').prop('disabled', false);
    } else {
        // Keep dropdowns disabled for purchase context
        $('#ddlSupplier').prop('disabled', true);
        $('#ddlSourceOfSupply').prop('disabled', true);
        $('#ddlDestinationOfSupply').prop('disabled', true);
    }
}
...
function updateSalePrice(id) {
    if (id != undefined) {
        ...
        //let PurchaseExcTax = parseFloat($('#txtPurchaseExcTax' + id).val());
        //$('#txtSalesExcTax' + id).val(Math.round((PurchaseExcTax + ((ProfitMargin / 100) * PurchaseExcTax)) * 100) / 100);
    }
    updateAmount();
}
...
function insert(i) {
    ...
    var det = {
        ...
        PurchaseAdditionalCharges: additionalCharges,
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                $('#txtNewCreditLimit').val(data.Data.User.CreditLimit + (data.Data.User.TotalPurchase - (data.Data.User.CreditLimit - data.Data.User.TotalPurchaseDue)));
                $('#lblCreditLimitHeader').text(data.Message);
                $('#hdnSupplierId').val(data.Data.User.UserId);
...
function update(i) {
    ...
    $('#divAdditionalCharges .additional-charges-row').each(function (index) {
        ...
        var additionalChargeId = $('#txtAdditionalChargeId' + count).val();
        var taxExemptionId = $('#exemptionReasonId' + count).val();
        var purchaseAdditionalChargesId = $('#txtPurchaseAdditionalChargesId' + count).val();
        additionalCharges.push({
            AdditionalChargeId: additionalChargeId,
            TaxId: taxId,
            AmountExcTax: additionalChargesAmountExcTax,
...
function UpdatePurchaseStatus(id) {
    if ($('#creditLimitWarningModal').is(':visible') == true) {
        r = true;
    }
    else {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.Status == 0) {
                    ...
                    $('#lblCreditLimitHeader').text(data.Message);
                    $('#hdnSupplierId').val(data.Data.User.UserId);
                    $('#hdnSubmitType').val('statusUpdate');
                }
                else {
                    if (EnableSound == 'True') { document.getElementById('success').play(); }
                    toastr.success(data.Message);
        ...
        success: function (data) {
            ...
            $('#AvailableCreditsModal').one('shown.bs.modal', function () {
                ...
                if ($paymentDate.length > 0 && !$paymentDate.data('DateTimePicker')) {
                    $paymentDate.datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
...
function updateTotal() {
    ...
    $('#divCombo tr').each(function () {
        var _id = this.id.split('divCombo')[1];
        if (_id != undefined && $('#txtAmount' + _id).val() != '' && $('#txtAmount' + _id).val() != undefined) {
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (SubmitType == 'insert') {
                    ...
                    UpdatePurchaseStatus(_PurchaseId);
                }
            }
...
function deleteBillOfEntry(BillOfEntryId) {
    var r = confirm("Are you sure you want to delete?");
    if (r == true) {
        var det = {
        ...
        $("#divLoading").show();
        $.ajax({
            url: '/Purchase/BillOfEntryDelete',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function fetchAdditionalCharges() {
    ...
    $('[id^="txtAdditionalChargesAmountExcTax"]').each(function () {
        ...
        var purchaseAdditionalChargesId = $('#txtPurchaseAdditionalChargesId' + index).val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.Data.AdditionalCharges.length; i++) {
                ...
                var itcDiv = '<div id="divInputTaxCreditAC' + count + '" style="display:' + (CountryId == 2 ? '' : 'none') + '; margin-top:5px; align-items:center;">' +
                ...
                var purchaseAdditionalChargesIdValue = existingValue ? existingValue.purchaseAdditionalChargesId : data.Data.AdditionalCharges[i].PurchaseAdditionalChargesId;
                ...
                html = html +
                    ...
                    '<input type="hidden" id="txtPurchaseAdditionalChargesId' + count + '" value="' + purchaseAdditionalChargesIdValue + '">' +
                    ddlTax +
                    exemptionDiv +
                    itcDiv +
                    '</div>' +
