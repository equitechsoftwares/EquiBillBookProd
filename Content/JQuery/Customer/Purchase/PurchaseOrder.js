$(function () {
    ...
    $('#_PurchaseOrderDate').datetimepicker({ widgetPositioning: { horizontal: 'auto', vertical: 'bottom' }, format: DateFormat.toUpperCase() + ' ' + TimeFormat, defaultDate: new Date() });
...
var _purchaseorderId = 0, _CheckStockPriceMismatch = true, _i, taxList;
var _BranchId = 0;
var accountSubTypes;
...
function fetchList(PageIndex) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/purchaseorder/purchaseorderFetch',
        datatype: "json",
        data: det,
        type: "post",
...
function View(purchaseorderId) {
    var det = {
        purchaseorderId: purchaseorderId
    };
    $("#divLoading").show();
    $.ajax({
        url: '/purchaseorder/purchaseorderView',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function deleteCombo(id, purchaseorderDetailsId) {
    if (purchaseorderDetailsId != undefined) {
        ...
        //$("#divLoading").show();
        //$.ajax({
        //    url: '/purchaseorder/purchaseorderDetailsDelete',
        //    datatype: "json",
        //    data: det,
        //    type: "post",
        //    success: function (data) {
...
function deleteFullCombo(purchaseorderId) {
...
function insert(i) {
    ...
    var det = {
        ...
        purchaseorderDate: moment($("#txtPurchaseOrderDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),
        Status: $("#ddlPurchaseOrderStatus  option:selected").val(),
        ReferenceNo: $("#txtPurchaseOrderReferenceNo").val(),
        ...
        purchaseorderDetails: ItemDetails,
        ...
        PurchaseOrderAdditionalCharges: additionalCharges,
        SpecialDiscount: $('#txtSpecialDiscount').val()
    };
    $("#divLoading").show();
    $.ajax({
        url: '/purchaseorder/purchaseordersInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (data.Data.PurchaseSetting.AutoPrintInvoicePurchaseOrder == true) {
                    sessionStorage.setItem('InvoiceUrl', '/purchaseorder/invoice?InvoiceId=' + data.Data.PurchaseOrder.InvoiceId);
                }
                //}
                //else if ($("#ddlPurchaseStatus option:selected").val() == 'Pending') {
                //    if (data.Data.PurchaseSetting.AutoPrintInvoicePending == true) {
                //        sessionStorage.setItem('InvoiceUrl', '/purchaseorder/invoice?InvoiceId=' + data.Data.Purchase.InvoiceId);
                //    }
                //}
...
function update(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            ...
            ItemDetails.push({
                DivId: _id,
                purchaseorderDetailsId: $('#txtPurchaseDetailsId' + _id).val(),
                Quantity: $('#txtQuantity' + _id).val(),
                AmountExcTax: $('#txtAmountExcTax' + _id).val(),
                AmountIncTax: $('#txtAmountIncTax' + _id).val(),
                ItemDetailsId: $('#txtItemDetailsId' + _id).val(),
    ...
    $('#divAdditionalCharges .additional-charges-row').each(function (index) {
        ...
        var additionalChargeId = $('#txtAdditionalChargeId' + count).val();
        var taxExemptionId = $('#exemptionReasonId' + count).val();
        var purchaseOrderAdditionalChargesId = $('#txtPurchaseOrderAdditionalChargesId' + count).val();
        additionalCharges.push({
            AdditionalChargeId: additionalChargeId,
            TaxId: taxId,
            AmountExcTax: additionalChargesAmountExcTax,
            ...
            ITCType: $('#itcTypeValue' + count).val(),
            TaxExemptionId: taxExemptionId,
            PurchaseOrderAdditionalChargesId: purchaseOrderAdditionalChargesId,
            IsActive: true,
            IsDeleted: false
        })
    });
    ...
    var det = {
        purchaseorderId: window.location.href.split('=')[1].replace('#', ''),
        BranchId: $('#ddlBranch').val(),
        DeliveredTo: $('#txtDeliveredTo').val(),
        Discount: $("#txtDiscAll").val(),
        DiscountType: $('#ddlDiscAll').val(),
...
function Delete(purchaseorderId) {
    var r = confirm("This will delete permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            purchaseorderId: purchaseorderId,
...
function fetchActiveStates(type) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/purchaseorder/ActiveStates',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function UpdatePurchaseStatus(id, Status) {
    var r = confirm("This will mark the Purchase Order as " + Status + ". This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        $('.errorText').hide();
        $('[style*="border: 2px"]').css('border', '');
        ...
        $("#divLoading").show();
        $.ajax({
            url: '/purchaseorder/UpdatepurchaseorderStatus',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
                ...
                if (data.Status == 0) {
                    ...
                    if (data.Data.PurchaseSetting.AutoPrintInvoicePurchaseOrder == true) {
                        PrintInvoice('/purchaseorder/invoice?InvoiceId=' + data.Data.PurchaseOrder.InvoiceId);
                    }
                }
            },
            error: function (xhr) {
...
function fetchAdditionalCharges() {
    ...
    $('[id^="txtAdditionalChargesAmountExcTax"]').each(function() {
        ...
        var purchaseOrderAdditionalChargesId = $('#txtPurchaseOrderAdditionalChargesId' + index).val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.Data.AdditionalCharges.length; i++) {
                ...
                var purchaseOrderAdditionalChargesIdValue = existingValue ? existingValue.purchaseOrderAdditionalChargesId : data.Data.AdditionalCharges[i].PurchaseOrderAdditionalChargesId;
                ...
                html = html +
                    ...
                    '<input type="hidden" id="txtPurchaseOrderAdditionalChargesId' + count + '" value="' + purchaseOrderAdditionalChargesIdValue + '">' +
                    ddlTax +
                    exemptionDiv +
                    itcDiv +
                    '</div>' +
