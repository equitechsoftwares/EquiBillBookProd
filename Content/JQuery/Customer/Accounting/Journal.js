...
var accountSubTypes, expenseFor, counter = 100000;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                expenseFor = data.Data.Users.filter(user => user.UserType.toLowerCase() != "user");;
            }
        },
        error: function (xhr) {
            $("#divLoading").hide();
...
function addNew() {
    ...
    var _expenseFor = '<option value="0" selected>Select </option>';
    for (let i = 0; i < expenseFor.length; i++) {
        /*_expenseFor = _expenseFor + '<option value="' + expenseFor[i].UserId + '">' + expenseFor[i].Name + '<span>-' + expenseFor[i].MobileNo + '</span>' + ' </option>';*/
        _expenseFor = _expenseFor + '<option value="' + expenseFor[i].UserId + '">' + expenseFor[i].MobileNo + ' | ' + expenseFor[i].UserType + ' </option>';
    }
    ...
    var html = '<tr id="divCombo' + counter + '">' +
        ...
        '<td width="20%"> ' +
        '<div class="input-group">' +
        '<select class="form-control select2" id="ddlExpenseFor' + counter + '"> ' +
        _expenseFor +
        '</select>' +
        '</div>' +
        '</td> ' +
        '<td class="service">' +
        '<div class="form-group">' +
        '<input type = "number" class="form-control" id="txtDebit' + counter + '" onkeypress="return onlyDecimalKey(event)" onchange="updateTotal(' + counter + ',0)">' +
        '</div>' +
        '</td>' +
        '<td class="service">' +
        '<div class="form-group">' +
        '<input type="number" class="form-control" id="txtCredit' + counter + '" onkeypress="return onlyDecimalKey(event)" onchange="updateTotal(' + counter + ',1)">' +
        '</div>' +
        '</td>' +
        '<td>' +
...
function deleteCombo(id, JournalPaymentId) {
    if (JournalPaymentId != undefined) {
        ...
        //var det = {
        //    JournalPaymentId: JournalPaymentId
        //}
        //$("#divLoading").show();
        //$.ajax({
        //    url: '/accounts/JournalPaymentDelete',
        //    datatype: "json",
        //    data: det,
        //    type: "post",
        //    success: function (data) {
...
function deleteFullCombo(JournalId) {
...
function insert(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            JournalPayments.push({
                ...
                ExpenseFor: $('#ddlExpenseFor' + _id).val(),
    ...
    var det = {
        ...
        BranchId: $('#ddlBranch').val(),
        JournalPayments: JournalPayments
    };
    $("#divLoading").show();
    $.ajax({
        url: '/accounts/journalInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                if (i == 1) {
                    window.location.href = "/accounts/journal";
                }
                else {
                    window.location.href = "/accounts/journaladd";
                }
            }
        },
        error: function (xhr) {
...
function update(i) {
    ...
    $('#divCombo tr').each(function () {
        ...
        if (_id != undefined) {
            JournalPayments.push({
                JournalPaymentId: $('#hdnJournalPaymentId' + _id).val(),
                AccountId: $('#ddlAccount' + _id).val(),
                Notes: $('#txtNotes' + _id).val(),
                ExpenseFor: $('#ddlExpenseFor' + _id).val(),
                Debit: $('#txtDebit' + _id).val(),
    ...
    var det = {
        JournalId: window.location.href.split('=')[1].replace('#', ''),
        Date: moment($("#txtDate").val(), DateFormat.toUpperCase() + ' ' + TimeFormat + ' a').format('YYYY-MM-DDTHH:mm'),//$('#txtDate').val(),
        ReferenceNo: $('#txtReferenceNo').val(),
        Notes: $('#txtNotes').val(),
        IsActive: true,
...
function Delete(JournalId, referenceno) {
    var r = confirm("This will delete \"" + referenceno + "\" permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            JournalId: JournalId,
        };
...
function BulkInsert(i) {
    ...
    var det = {
        BatchNo: BatchNo,
        JournalImports: _excelJson
    }
    $.ajax({
        url: '/accounts/ImportJournal',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function fetchBulkInsertProgress() {
    ...
    $.ajax({
        url: '/accounts/JournalCountByBatch',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function FetchOtherSoftwareImport(PageIndex) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/accounts/OtherSoftwareImportFetch',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function DeleteOtherSoftwareImport(OtherSoftwareImportId) {
    ...
    if (r == true) {
        ...
        $("#divLoading").show();
        $.ajax({
            url: '/accounts/OtherSoftwareImportDelete',
            datatype: "json",
            data: det,
            type: "post",
            success: function (data) {
...
function InsertOtherSoftwareImport() {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/accounts/OtherSoftwareImportInsert',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
