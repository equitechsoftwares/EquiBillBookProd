    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Status == 0) {
                ...
                //window.location.href = "/itemsettings/Unit";
                if (i == 1) {
                    window.location.href = "/itemsettings/Unit";
                }
                else {
                    window.location.href = "/itemsettings/unitadd";
                }
            }
        },
        error: function (xhr) {
...
function update(i) {
    ...
    var det = {
        UnitId: window.location.href.split('=')[1],
        UnitCode: $('#txtUnitCode').val(),
        UnitName: $('#txtUnitName').val(),
        UnitShortName: $('#txtUnitShortName').val(),
        AllowDecimal: $('#ddlAllowDecimal').val(),
...
function ActiveInactive(UnitId, IsActive) {
    var det = {
        UnitId: UnitId,
        IsActive: IsActive
    };
    $("#divLoading").show();
    $.ajax({
        url: '/itemsettings/UnitActiveInactive',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
...
function Delete(UnitId, UnitName) {
    var r = confirm("This will delete \"" + UnitName + "\" permanently. This process cannot be undone. Are you sure you want to continue?");
    if (r == true) {
        var det = {
            UnitId: UnitId,
        };
