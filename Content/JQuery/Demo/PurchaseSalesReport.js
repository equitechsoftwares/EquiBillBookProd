...
function fetchList(PageIndex) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/reports/purchasesalesFetch',
        datatype: "json",
        data: det,
        type: "post",
...
function exportToExcel() {
    ...
    TableToExcel.convert(document.getElementById("tblData"), {
        name: "Purchase & Sales Report.xlsx",
        sheet: {
            name: "Sheet1"
        }
    });
...
function exportToCsv() {
    $('.hide').remove();
    TableToExcel.convert(document.getElementById("tblData"), {
        name: "Purchase & Sales Report.csv",
        sheet: {
            name: "Sheet1"
        }
    });
...
function exportToPdf() {
    ...
    html2canvas($('#tblData')[0], {
        onrendered: function (canvas) {
            ...
            pdfMake.createPdf(docDefinition).download("Purchase & Sales Report.pdf");
