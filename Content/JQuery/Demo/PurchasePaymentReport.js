...
function fetchList(PageIndex) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/reports/SupplierPaymentFetch',
        datatype: "json",
        data: det,
        type: "post",
...
function exportToExcel() {
    ...
    TableToExcel.convert(document.getElementById("tblData"), {
        name: "Purchase Payment Report.xlsx",
        sheet: {
            name: "Sheet1"
        }
    });
...
function exportToCsv() {
    $('.hide').remove();
    TableToExcel.convert(document.getElementById("tblData"), {
        name: "Purchase Payment Report.csv",
        sheet: {
            name: "Sheet1"
        }
    });
...
function exportToPdf() {
    ...
    html2canvas($('#tblData')[0], {
        onrendered: function (canvas) {
            ...
            pdfMake.createPdf(docDefinition).download("Purchase Payment Report.pdf");
