...
function exportToPdf() {
    ...
    html2canvas($('#tblData')[0], {
        onrendered: function (canvas) {
            ...
            pdfMake.createPdf(docDefinition).download("Profit Loss.pdf");
...
function checkType(_type) {
    ...
    $("#divLoading").show();
    $.ajax({
        url: '/reports/ProfitLossByAttributesFetch',
        datatype: "json",
        data: det,
        type: "post",
        success: function (data) {
            $("#divProfitLossAttributes").html(data);
            $('.chkIsActive').bootstrapToggle();
            $("#divLoading").hide();
